...
class PastebinAPI(object):
    ...
    get_paste_content -- Returns the raw content of paste by user    	
    ...
    # URL to the Login API
    _api_raw_url = 'http://%s/api/api_raw.php' % _base_domain
    ...
    def get_paste_content(self, api_dev_key, api_user_key, api_paste_key):
        """Returns all pastes for the provided api_user_key.
        ...
        Usage Example::
            >>> from pastebin import PastebinAPI
            >>> x = PastebinAPI()
            >>> content = x.get_paste_content('453a994e0e2f1efae07f8759e59e075b',
            ...                         'c57a18e6c0ae228cd4bd16fe36da381a',
            ...                         '33NZzFCk')
            >>> print content
            ACTUAL TEXT CONTENT
        ...
        @type   api_paste_key: string
        @param  api_paste_key: The API Paste key of pastes from user

        @rtype: string
        @returns: Returns the raw content of the pastes
        """
        ...
        # Valid API option - 'paste' is default for new paste
        argv['api_option'] = str('show_paste')

        # lets try to read the URL that we've just built.
        request_string = urllib.urlopen(self._api_raw_url, urllib.urlencode(argv))
        response = request_string.read()
